  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <main className="mx-auto w-full max-w-3xl space-y-4 px-4 py-4">
        <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
          ...
          <div className={`grid grid-cols-2 gap-2 sm:grid-cols-4 ${showAdvanced ? '' : 'hidden sm:grid'}`}>
            ...
            <button onClick={refreshCsv} className="h-10 w-full rounded-md bg-brand-700 px-3 text-sm font-semibold text-white">
              새로고침
            </button>
          </div>
        </section>
        ...
        {pinnedFavorites.length > 0 && (
          <section className="space-y-3">
            <div className="rounded-md border border-slate-300 bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">
      ...
      <button
        ...
        className="fixed bottom-4 right-4 rounded-md bg-brand-700 px-3 py-2 text-xs font-semibold text-white"
      >
        맨 위로
      </button>
